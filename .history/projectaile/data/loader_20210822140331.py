class LOADER:
    ...
    def get_batch_info(self, mode='train', itertator=0, batch_size=1):
        if mode == 'train':
            indices = self.train_indices
            features = self.train_features
            targets = self.train_targets
            indices = self.train_indices[iterator*batch_size:(iterator+1)*batch_size]
            return self.train_features[indices], self.train_targets[indices]
        elif mode == 'valid':
            indices = self.valid_indices
            features = self.valid_features
            targets = self.valid_targets
            
            return self.valid_features[indices], self.valid_targets[indices]
        else:
            return None, None